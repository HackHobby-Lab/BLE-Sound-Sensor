...
#define DEBOUNCE_SECONDS_DEFAULT      3u   /* sustain needed for alert #1 */
#define WINDOW_SECONDS_DEFAULT       30u   /* unused in new logic */
#define ALERT_PERIOD_SECONDS_DEFAULT  5u   /* nag interval */

#define NAG_AFTER_TRIGGERS            3u   /* alerts before nag mode kicks in */
#define HYSTERESIS_DB                 5    /* drop this far below threshold to count as a dip */
#define IDLE_RESET_SECONDS           30u   /* quiet this long -> full reset */
#define RISING_GRACE_MS             700    /* allowed micro-pauses while RISING */
...
int main(void)
{
	...
	printk("\nMic-Sense ready  |  %d Hz, %d-bit, left channel\n",
	       SAMPLE_FREQ, SAMPLE_BIT_WIDTH);
	printk("Escalation rule:\n");
	printk("  alert #1 -> SPL >= %u dB sustained %us\n",
	       threshold_value, debounce_seconds);
	printk("  alerts #2 & #3 -> on each new crossing of %u dB (after dipping below %d dB), or every %us continuous loud\n",
	       threshold_value, (int)threshold_value - HYSTERESIS_DB, alert_period_s);
	printk("  after alert #%u -> NAG mode: every %us while loud\n",
	       NAG_AFTER_TRIGGERS, alert_period_s);
	printk("  reset -> after %us below %d dB\n",
	       IDLE_RESET_SECONDS, (int)threshold_value - HYSTERESIS_DB);
	...
	while (1) {
		...
		bool quiet_now = (db_int <  (int16_t)threshold_value - HYSTERESIS_DB);
...
#if !DATA_COLLECT_MODE
		{
			...
			if (++stat_div >= 10) {
				...
				printk("[STAT] SPL=%d thr=%u state=%s "
				       "triggers=%u/%u stream_sub=%d alert_sub=%d\n",
				       (int)db_int, threshold_value, state_str,
				       (unsigned)det_trigger_count,
				       (unsigned)NAG_AFTER_TRIGGERS,
		...
		case DET_IDLE:
			if (loud) {
				...
				printk("[CRY] rising — SPL=%d, counting toward %us\n",
		...
		case DET_RISING:
			if (loud) {
				...
				det_state = DET_IDLE;
				printk("[CRY] rising aborted (dropped during debounce)\n");
				break;
			}
		...
		case DET_ACTIVE:
			if (quiet_now) {
				/* SPL dropped below hysteresis floor */
				det_dipped_since_alert = true;
				if (det_quiet_since_ms == 0) {
					det_quiet_since_ms = now_ms;
				}
				/* Long quiet → end the event entirely */
				if (now_ms - det_quiet_since_ms >=
				    (int64_t)IDLE_RESET_SECONDS * 1000) {
					alertThreshold = 0;
					if (alert_notify_enabled) {
					...
					printk("[CRY] event ended — quiet for %us, resetting\n",
					       IDLE_RESET_SECONDS);
					det_state              = DET_IDLE;
					det_trigger_count      = 0;
					det_dipped_since_alert = false;
				...
				/* Above threshold (not just in hysteresis band) */
				det_quiet_since_ms = 0;
				bool fire_now = false;

				if (det_trigger_count < NAG_AFTER_TRIGGERS) {
					/* Pre-nag: re-fire on each new rise after a dip,
					 * or every `alert_period_s` if continuously loud. */
					if (det_dipped_since_alert) {
						fire_now = true;
						det_dipped_since_alert = false;
					} else if (now_ms - det_last_alert_ms >=
						   (int64_t)alert_period_s * 1000) {
						fire_now = true;
					}
				} else {
					/* Nag mode: fire every alert_period_s while loud */
				...
				if (fire_now) {
					...
					det_last_alert_ms = now_ms;
					if (det_trigger_count == NAG_AFTER_TRIGGERS) {
						printk("[CRY] entering NAG mode — alerts every %us while loud\n",
						       alert_period_s);
					}
				}
			}
			/* In hysteresis band (not loud, not quiet): hold state, do nothing */
			break;
		}
